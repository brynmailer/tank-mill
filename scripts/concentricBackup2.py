...
# Store edge information for circle center calculation - ONLY for points that should get circles
circle_points_info = []
...
for i, ((inner_p1, inner_p2), (exp_p1, exp_p2), base_edge) in enumerate(zip(inner_edges, expected_edges, edges)):
    ...
    # Store edge info ONLY for middle points (exclude actual start/end points of edge)
    # If num > 2, the middle points are exp_eq[1:-1]
    if num > 2:
        for pt in exp_eq[1:-1]:  # exclude first and last (actual endpoints)
    ...
    # Store edge info for endpoint offset points (these DO get circles)
...
# First calculate the center of the mirrored polygon
edges_m_temp = [flip_points([p1, p2]) for p1, p2 in edges]
edges_m_temp = [(pts[0], pts[1]) for pts in edges_m_temp]
all_vertices_m = []
for p1, p2 in edges_m_temp:
    all_vertices_m.extend([p1, p2])
unique_vertices_m = list(set(all_vertices_m))
centroid_m = np.mean(unique_vertices_m, axis=0)

circle_centers_m = []
for i, (pt, exp_p1, exp_p2) in enumerate(circle_points_info):
    ...
    # Calculate direction toward mirrored polygon center
    pt_m_array = np.asarray(pt_m, dtype=float)
    toward_center_m = unit_vector(centroid_m - pt_m_array)
    center_m = tuple(pt_m_array + 64 * toward_center_m)
    circle_centers_m.append(center_m)
...
# Add circles ONLY for middle points and endpoint offsets (not actual endpoints)
...
ax1.set_title(f"A Data: Concentric Shapes and Points\n"
              ...
              f"64mm circles only on middle points and z-offset endpoints")
ax1.grid(True)
...
# Add circles ONLY for middle points and endpoint offsets (mirrored)
...
print(f"64mm circles only on middle points and z-offset endpoints (not actual edge endpoints)")
print(f"Total circles created: {len(circle_centers)} per side")