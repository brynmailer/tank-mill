...
# ---- Settings ----
path = "./data/a_points_testing.csv"
...
with open("./data/a_tank_mill_features.json", "r") as f:
...
feed_linear = 1500.0        # cutting moves (G1 XY/XYZ)
feed_linear_outcut = 500    # decreese speed for first outsid pass
...
with open("tank_full_job_warped.gcode", "w") as f:
    ...
    f.write("\n( Spiral-milled holes: helical 2mm per rev, mid-lift at half depth )\n")
    chip_clear_dwell_s = 0.4  # optional pause after lift
    ...
    for i, (x, y) in enumerate(warped_drill_points, 1):
        z_top        = float(drill_surface_z[i-1])   # local surface at hole
        target_depth = final_plane_z                 # final Z for all holes
        start_x, start_y = x + offset, y            # helical start point (radius = offset)
        ...
        current_depth = z_top
        half_depth = 0.5 * (z_top + target_depth)   # halfway between surface and target
        did_mid_lift = False

        while current_depth > target_depth + 1e-9:
            next_depth = max(current_depth - spiral_stepdown, target_depth)

            # One full CCW circle returning to (start_x,start_y) while stepping Z down
            f.write(
            ...
            # Mid-lift once, right after we pass the halfway depth
            if (not did_mid_lift) and (current_depth <= half_depth + 1e-9):
                write_rapid(f, z=z_top + approach_height + probe_offset_z)  # lift to clear chips
                f.write(f"G4 P{chip_clear_dwell_s:.2f}\n")                  # optional dwell
                write_rapid(f, x=start_x, y=start_y)                        # ensure we’re back at start of helix
                f.write(                                                     # plunge back to the current cut depth
                    f"G1 Z{current_depth + probe_offset_z:.3f} F{feed_plunge:.0f}\n"
                )
                did_mid_lift = True
    ...
    for i, (x, y) in enumerate(warped_drill_points, 1):
        ...
        target_depth = final_plane_z            # final Z
