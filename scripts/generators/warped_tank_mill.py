...
def apply_switch_offsets_to_points(points, mirror, switch_offsets, points_per_face):
    """
    Apply per-switch Z offsets to probed points.

    If `points` is a pandas DataFrame with columns x,y,z, it returns a DataFrame
    of the same shape and columns, only Z modified.
    If `points` is an array-like, it returns a NumPy array with modified Z.

    Mapping (0-based indices into switch_offsets):

        mirror = True   → [0, 1, 2, 3, 4, 5, 6]
        mirror = False  → [7, 1, 6, 5, 4, 3, 2]
    """
    is_df = isinstance(points, pd.DataFrame)
    ...
    n_points = len(pts)
    expected_points = sum(points_per_face)

    if n_points != expected_points:
        raise ValueError(
            f"Number of points ({n_points}) does not match sum(points_per_face) "
            f"({expected_points})."
        )

    # Define switch order per face
    if mirror:
        # mirror: face 0..6 → switch indices [0,1,2,3,4,5,6]
        switch_order = list(range(len(points_per_face)))  # assuming len(points_per_face) == 7
    else:
        # non-mirror: face 0..6 → switch indices [7,1,6,5,4,3,2]
    ...
    if max(switch_order) >= len(switch_offsets):
        raise ValueError(
            ...
            f"but switch_order requires index {max(switch_order)}"
        )

    idx = 0
    for face_idx, count in enumerate(points_per_face):
        start = idx
        end = idx + count

        switch_idx = switch_order[face_idx]
...
parser.add_argument("-m", "--mirror", action="store_true",
                    help="Enable mirror mode (defaults to off)")
parser.add_argument("path", type=str,
                    help="Path to points csv")
...
groove_depth = 5.5           # Groove cut depth (mm)
outcut_depth = 16          # Outcut depth (mm)
outcut_depth_bottom = 17.0 
...
file_path = ("./data/b_tank_mill_features.json"
    if args.mirror
    else "./data/a_tank_mill_features.json")
...
#JSON format to tuple format expected by utils.build_path
...
# ---- Pick arcs depending on mirror ----
if args.mirror:
    groove_arc, outcut_arc = utils.arc_g3, utils.arc_g2
else:
    groove_arc, outcut_arc = utils.arc_g2, utils.arc_g3

# ---- Groove path ----
print("Building groove path")
groove_x, groove_y = utils.build_path(groove_segments, groove_arc)
...
outcut_x, outcut_y = utils.build_path(outcut_segments, outcut_arc)
...
# ---- 4. Groove Warping ----
baseline = 74 # distance between probe point and grove = 16 + 54.8 was 80
...
spiral_stepdown = 4.0   # mm per spiral
hole_diameter   = 8.5
drain_drill     = 8.0
tool_diameter   = 8.0
...
# ---- Feeds (mm/min) ----
feed_plunge = 300.0         # Z-only plunges / re-plunges
feed_linear = 2600.0        # cutting moves (G1 XY/XYZ)
feed_linear_outcut = 2300    # decreese speed for first outsid pass
...
# Utility: buil10.1.4.243d “parallel-to-surface” Z paths for a given total depth & step
...
with open("tank_full_job_warped.gcode", "w") as f:
    ...
    # Cut groove if mirrored or using double-seal; otherwise skip.
        # --- Groove cutting logic ---
    # Cut groove if mirrored or using double-seal; otherwise skip.
    # SINGLE PASS: go straight to final groove depth following the tank surface.
    if args.mirror or use_double_seal:
        # Final single-pass Z profile (surface minus groove depth)
        final_groove_z = surface_z - groove_depth

        # Wrap in a list because write_closed_groove_passes expects a list of passes
        groove_passes = [final_groove_z]

        write_closed_groove_passes(
            f,
            smooth_groove_x,
            smooth_groove_y,
            groove_passes,
...
last_pass_surface = groove_passes_plot[-1]
